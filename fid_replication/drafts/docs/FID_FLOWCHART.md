...
A single rendered view of fid processing as currently understood. Every node
and edge here corresponds to a documented step in the prose docs; nothing is
fabricated. For narrative detail, derivations, citations, and parameter
tables, follow the cross-references in §3.


## 1. Diagram

![Fid processing flowchart](FID_FLOWCHART.svg)
...
This diagram is a visual index over three prose docs. For every subgraph,
the doc listed below is authoritative; the C source citations live there.

| Subgraph | Authoritative section |
|---|---|
| CALDB + Per-observation reduction | [FID_LIGHT_PROCESSING.md §4](FID_LIGHT_PROCESSING.md), [FID_CORRECTION_STAGES.md §2-§3](FID_CORRECTION_STAGES.md), [PREDICTED_FID_AND_ASP_SOLVE.md §6](PREDICTED_FID_AND_ASP_SOLVE.md) (`Read_CALALIGN_Write_Fidprops` and the LSI polynomial correction) |
| Per-sample measured chain | [FID_LIGHT_PROCESSING.md §5-§6](FID_LIGHT_PROCESSING.md), [FID_CORRECTION_STAGES.md §4-§6](FID_CORRECTION_STAGES.md) |
| Predicted chain | [PREDICTED_FID_AND_ASP_SOLVE.md §2](PREDICTED_FID_AND_ASP_SOLVE.md) |
| asp_solve fit + ASPSOL | [PREDICTED_FID_AND_ASP_SOLVE.md §3-§5](PREDICTED_FID_AND_ASP_SOLVE.md), [ASPSOL_PRODUCTS.md](ASPSOL_PRODUCTS.md) |

Pipeline-step ordering (`aca_read_data` → `aca_corr_ccd` → `aca_calc_centr`
→ `aca_corr_centr` → `aca_filter_centr` → `aca_corr_fid` → `asp_solve`)
is set by [runasp/runasp.py:20-56](../runasp/runasp.py#L20-L56).

## 4. What this diagram deliberately omits

- The **fid identification** machinery inside `aca_id_image` that decides
  which ACA slots carry fids (and which carry stars). Treated here as a
  precondition; see `aca_id_image.cc` driver and `Aca_Id_Image.cc` for the
  matching logic.
- The **sigma-reject status bits** set by `aca_filter_centr`
  (`SIGMA_REJECT_Y/Z/M`) — they do not change the data path, only the
  per-sample `status` flag. Documented in
  [FID_CORRECTION_STAGES.md §5](FID_CORRECTION_STAGES.md).
- The **OBI / CAI** distinction in the asp_solve output and the
  `asp_combine` post-processing step. See
  [ASPSOL_PRODUCTS.md](ASPSOL_PRODUCTS.md).
- The **guide-star half** of `aca_corr_centr`, which uses the `_star`
  variants of the same distortion polynomial and feeds the KALMAN attitude
  filter rather than the fid fit.
