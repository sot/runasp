"""Fid deltas the way the Ska/mica tooling computes them, as a cross-check.

Same predicted chain as `fid_processing.estimated_fid_positions`, reached by a
different route: the alignment matrices come from the observation's own ACACAL
product rather than from the CALDB alignment file, the frame offsets come from
the FIDPROPS header keywords, and the result is differenced against the
measured `ang_y_sm` / `ang_z_sm` to give a residual per centroid sample.

Two differences from `fid_processing` that are worth knowing and do not matter:

- the alignment product here is ACA_MISALIGN . FTS_MISALIGN, two matrices
  rather than three. ACA_SC_ALIGN, the missing one, is exactly the identity in
  every CALALIGN file, so the two products agree.
- the direction cosine is not normalised before the arctangents are taken,
